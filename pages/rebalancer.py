...
def rebalancer():
    ...
    if not current_portfolio.empty:
        # check if row has compelte data    
        if not current_portfolio.isnull().values.any():
            st.session_state.rebalance = current_portfolio
            ...
            if current_portfolio['Target Weight %'].sum() == 100:
                ...
                prices = []
                for ticker in edited_df['Ticker']:
                    prices.append(tickers.tickers[ticker].history(period='1d').iloc[-1]['Close'])
                edited_df['Price'] = prices
                ...
                for idx in edited_df.index:
                    ...
                    future_weight = edited_df.loc[idx, 'future_weight']
